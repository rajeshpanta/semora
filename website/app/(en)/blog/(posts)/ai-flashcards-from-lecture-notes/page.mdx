...
export const faqItems = [
  {
    ...
    answer:
      'Yes. Every tool in this comparison accepts uploaded material — PDFs, slide decks, notes, and in some cases recordings and video — and returns question-and-answer cards from it. Generation takes seconds. The work that determines whether the deck is useful happens afterward: cutting the cards that test nothing, splitting any card whose answer runs longer than a sentence or two, and checking the facts against your own notes before the first review.',
  },
  {
...
The first is **retrieval practice**: the act of pulling an answer out of memory strengthens that memory more than reviewing the same material does. This is why a card with the answer visible on the front is not a flashcard — it is a note. The retrieval attempt is the entire mechanism, and a card that does not force one is doing nothing.

The second is the **spacing effect**: information reviewed at increasing intervals is retained substantially longer than the same total review time crammed together. This is why the schedule matters as much as the deck. Reviewing 100 cards once the night before is not the same intervention as reviewing 30 cards five times across two weeks, even if the total minutes are similar.
