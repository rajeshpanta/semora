-- ============================================================
-- SEMORA LMS SUBMISSION INTEGRITY
-- Course-scoped external ids + accurate completion timestamps.
-- ============================================================

-- Google Classroom and some self-hosted LMS installations only guarantee an
-- assignment id inside a course. Keep the source key course-scoped.
alter table public.tasks
  drop constraint if exists tasks_lms_external_unique;
alter table public.tasks
  add constraint tasks_lms_external_unique
...
begin
  ...
  loop
    ...
    insert into public.tasks (
      ...
      item_completed,
      case when item_completed then coalesce(item_completed_at, now()) else null end,
      item_submitted_late,
    ...
    on conflict on constraint tasks_lms_external_unique do update set
      ...
      completed_at = case
        ...
        when excluded.is_completed then coalesce(excluded.completed_at, public.tasks.completed_at, now())
  ...
  -- Compare within each external course. Assignment ids from another course
  -- must never keep a removed row falsely alive.
