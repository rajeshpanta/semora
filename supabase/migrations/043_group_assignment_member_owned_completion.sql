-- ============================================================
-- SEMORA GROUP-ASSIGNMENT COMPLETION IS MEMBER-OWNED
-- Migration 041 made the shared group assignment's completion AUTHORITATIVE over
-- each member's personal synced task, overwriting is_completed on every re-sync.
-- Combined with CollaborationSyncBridge (which re-syncs on foreground + realtime),
-- that silently reverted a member's own completion: a student checks off their
-- copy of a group task in the planner, and the next sync flips it back.
--
-- Fix: completion is MEMBER-OWNED, exactly like shared_deadlines already are
-- (their on-conflict update deliberately never touches is_completed). The sync
-- still creates the personal task and keeps its DEADLINE fields (title, dates,
-- description) current, and a brand-new task still adopts the group's completion
-- state on first insert — but a re-sync never clobbers the member's own checkmark.
-- The group assignment's collective is_completed remains its own field, surfaced
-- in the collaboration screen, decoupled from each member's personal progress.
-- ============================================================
